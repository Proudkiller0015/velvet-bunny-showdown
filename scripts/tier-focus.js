...
const { rate, outsideChance } = require('../src/tier-sim/rating');
...
const ranked = outsideChance(fit.rows, fit.tierMean).sort((a, b) => b.p - a.p);
...
console.log(`${good.length} games. Kept ${keep.length} of ${ranked.length} as focus (outside-band chance >= ${Math.round(100 * (keep.length ? keep[keep.length - 1].p : 0))}%).`);
